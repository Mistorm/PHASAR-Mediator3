...
public class Query {
    ...
    public List<SuggestionItem> getSuggestion(JSONObject jsonObject, String position) throws JSONException {

	Triple filterTriple = null;

	if (position.equals("relator")) {
	    //Argument jsonObject must contain a arrow
	    filterTriple = new Triple(quadList.getBoxContent(jsonObject.getInt("a")),
		    WILDCARD,
		    quadList.getBoxContent(jsonObject.getInt("b")),
		    quadList.getBoxContent(jsonObject.getInt("direction")));
	} else if (position.equals("head")) {
	    //Argument jsonObject must contain a box
	    //triple = new Triple(jsonObject.getString("a"), jsonObject.getString("relator"), jsonObject.getString("b"), jsonObject.getString("direction"));
	} else if (position.equals("tail")) {
	    
	} else {
	    throw new IllegalArgumentException("Argument position not 'relator', 'head' of 'tail'.");
	}

	List<Triple> matches = matchTriples(filterTriple);
